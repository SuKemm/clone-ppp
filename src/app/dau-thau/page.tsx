...
// Trang danh sách Đấu thầu — dựng theo đúng khuôn của /tin-tuc (xem
// src/app/tin-tuc/page.tsx) vì 2 nội dung có hình dạng giống nhau (chuyên
...
function buildHref(category: string, page: number) {
  ...
  return qs ? `/dau-thau?${qs}` : "/dau-thau";
}
...
export default async function TendersPage({
  ...
  const filtered = activeTab === "Tất cả" ? tenders : tenders.filter((item) => item.category === activeTab);
  ...
  // Sidebar "Xem nhiều nhất" — sắp xếp theo lượt xem thật, giảm dần (không
  // phải chỉ lấy bài mới nhất), giống cách đã làm ở trang chi tiết tin tức.
  ...
  return (
    <PtscShell title="Đấu thầu" description="Thông báo mời thầu, kế hoạch và kết quả lựa chọn nhà thầu.">
      <section className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
        <h1 className="text-3xl font-bold uppercase tracking-wide text-slate-900">Đấu thầu</h1>
        ...
        {filtered.length === 0 && (
          <p className="mt-10 text-center text-slate-500">Chưa có thông báo đấu thầu nào trong chuyên mục này.</p>
        )}
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_320px]">
          <div>
            {featured && (
              <article className="group">
                <Link href={`/dau-thau/${featured.id}`}>
            ...
            {totalPages > 1 && (
              <nav aria-label="Phân trang đấu thầu" className="mt-14 flex flex-wrap items-center justify-center gap-2">
                <Link
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="mt-5 space-y-5">
              {sidebarItems.length === 0 && <p className="text-sm text-slate-500">Chưa có thông báo nào.</p>}
              {sidebarItems.map((item) => (
                <Link key={item.id} href={`/dau-thau/${item.id}`} className="group flex gap-3">
            ...
            <Link href="/dau-thau" className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold text-cyan-700 hover:underline">
              Xem tất cả đấu thầu
