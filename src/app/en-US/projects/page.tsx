...
// Đồng bộ với trang tiếng Việt /du-an — cùng đọc từ "projects", chỉ khác
// lấy các field "_en" (title_en, category_en, description_en) và đường
// link/nhãn đổi sang bản EN (/en-US/projects).
...
function buildHref(page: number) {
  return page > 1 ? `/en-US/projects?page=${page}` : "/en-US/projects";
}
  ...
  return (
    ...
    >
      <section className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          ...
          {pageItems.map((project) => {
            ...
            return (
              <article key={project.id} className="group rounded-[1.5rem] border border-slate-200 bg-white p-8 shadow-sm transition hover:shadow-md">
                <Link href={`/en-US/projects/${project.id}`}>
                  {project.image && (
                  ...
                  <span className="mt-4 inline-block text-sm font-semibold text-cyan-700">Read more →</span>
                </Link>
              </article>
            );
          })}
        ...
        {totalPages > 1 && (
          <nav aria-label="Projects pagination" className="mt-14 flex flex-wrap items-center justify-center gap-2">
            <Link
